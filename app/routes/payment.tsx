  ...
  return (
    <section className="min-h-screen bg-slate-100 py-20 px-5">
      <div className="absolute top-0 left-0 p-5 z-10">
        <button>
          <Menu size={24} color="black" onClick={() => setShowSidebar(true)} />
        </button>
      </div>
      ...
      {!amount ? (
        <>
          <p className="mx-auto max-w-lg mt-10">
            ...
            <span className="underline">Debit / Credit Card</span>, please enter
            the payment amount in the box below.
          </p>
          <div className="w-full max-w-lg mx-auto">
            <input
              type="number"
              value={userSelectedAmount}
              onChange={(e) => setUserSelectedAmount(parseInt(e.target.value))}
